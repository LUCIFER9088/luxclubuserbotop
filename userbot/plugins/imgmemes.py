...
# credits to @TeamLuxBot (@TeamLuxBot)
import asyncio
import os
import re
...
@lionub.lion_cmd(
    ...
    info={
        ...
        "examples": "{tr}fakegs LuxBot ; One of the Popular userbot",
    },
)
async def nekobot(lion):
    ...
    cate = await edit_or_reply(lion, "`Connecting to https://www.google.com/ ...`")
    text = deEmojify(text)
    if ";" in text:
...
@lionub.lion_cmd(
    ...
    info={
        ...
        "usage": "{tr}trump <text>",
        "examples": "{tr}trump LuxBot is One of the Popular userbot",
    },
)
async def nekobot(lion):
    ...
    cate = await edit_or_reply(lion, "`Requesting trump to tweet...`")
    text = deEmojify(text)
    await asyncio.sleep(2)
...
@lionub.lion_cmd(
    ...
    info={
        ...
        "usage": "{tr}modi <text>",
        "examples": "{tr}modi LuxBot is One of the Popular userbot",
    },
)
async def nekobot(lion):
    ...
    cate = await edit_or_reply(lion, "Requesting modi to tweet...")
    text = deEmojify(text)
    await asyncio.sleep(2)
...
@lionub.lion_cmd(
    ...
    info={
        ...
        "usage": "{tr}cmm <text>",
        "examples": "{tr}cmm LuxBot is One of the Popular userbot",
    },
)
async def nekobot(lion):
    text = lion.pattern_match.group(1)
    ...
    cate = await edit_or_reply(lion, "`Your banner is under creation wait a sec...`")
    text = deEmojify(text)
    await asyncio.sleep(2)
...
@lionub.lion_cmd(
    ...
    info={
        ...
        "usage": "{tr}kanna text",
        "examples": "{tr}kanna LuxBot is One of the Popular userbot",
    },
)
async def nekobot(lion):
    ...
    cate = await edit_or_reply(lion, "`Kanna is writing your text...`")
    text = deEmojify(text)
    await asyncio.sleep(2)
...
@lionub.lion_cmd(
    ...
    info={
        ...
        "examples": "{tr}tweet iamsrk ; LuxBot is One of the Popular userbot",
    },
)
async def nekobot(lion):
    ...
    if ";" in text:
        ...
        return
    cate = await edit_or_reply(lion, f"`Requesting {username} to tweet...`")
    text = deEmojify(text)
    await asyncio.sleep(2)
