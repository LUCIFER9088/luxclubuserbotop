...
async def cmdlist():
    outstr = "**Total list of Commands in your LuxBot are :**\n\n"
    category = ["admin", "fun", "tools", "utils"]
    for lion in category:
        plugins = GRP_INFO[lion]
        outstr += f"**{hemojis[lion]} {lion.title()} ** - {len(plugins)}\n\n"
...
@lionub.lion_cmd(
    ...
    info={
        "header": "To get guide for LuxBot.",
...
async def _(event):
    ...
    if not input_str:
        ...
        outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help -c <command name>`"
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of LuxBot are :"
    )
...
async def _(event):
    ...
    if found:
        ...
        out = f"I can't find any such command `{cmd}` in LuxBot"
