...
LIONLOGO = "https://telegra.ph/file/d01bc3a1b9398d62136e2.jpg"
...
def main_menu():
    text = f"𝗟𝗨𝗫𝗖𝗟𝗨𝗕 𝗨𝗦𝗘𝗥𝗕𝗢𝗧'𝗌 𝗛𝗲𝗹𝗽 𝗠𝗲𝗻𝘂\
        \n𝗣𝗿𝗼𝘃𝗶𝗱𝗲𝗿 {mention}"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**LionX"):
            ...
            _result = main_menu()
            result = builder.article(
                title="© LUXCLUB SERGIO",
                description="Help menu for LUXCLUB USERBOT",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        text, msg_entities = await event.client._parse_message_text(
            "𝐃𝐞𝐩𝐥𝐨𝐲 𝐘𝐨𝐮𝐫 𝐎𝐰𝐧 𝐔𝐒𝐄𝐑𝐁𝐎𝐓 (𝑳𝒊𝒐𝒏𝑿 𝑵𝒐𝒐𝒃𝒅𝒂) .", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
